...
# ─────────────────────────────────────────────────────────────────────────────
def transform_amount_column(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize the amount column to absolute values.

    Some downstream processing may expect all amounts to be non-negative
    (e.g. when only the sign is recorded in a separate "type" column). This
    utility applies `abs()` to every entry in the "amount" column and
    returns the modified dataframe. The operation is performed in place.
    """
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_num, page in enumerate(pdf.pages, start=1):
            ...
            for match in TRANSACTION_RE.finditer(text):
                ...
                if debug:
                    logger.debug(f"  Matched: {date_raw.strip()} | {description} | {amount}")

    if not transactions:
        return pd.DataFrame()
    df = pd.DataFrame(transactions).reset_index(drop=True)
    df["amount"] = df["amount"].abs()  # ensure amounts are positive; type indicates credit/debit
    return df
...
def main():
    ...
    for file in files:
        ...
        if engine is not None:
            df.to_sql("capital_one", engine,schema="statements", if_exists="append", index=False)
        ...
        try:
            ...
            logger.warning(f"File exists in archive or error moving: {e}")
